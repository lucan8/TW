...
db.connect(async (e)=>{
    ...
    db.query(`CREATE DATABASE IF NOT EXISTS task_hero`, async (e) =>{
        if (e)
            console.log(e);
        else
            console.log('Created database task_hero!');

        //Creating a connection pool and all the tables needed
        pool = mysql.createPool(sql_options).promise();   
        createTableUsers().then(createTableDailyQuests).then(createTableDailyQuestsComp);
        ...
        var sessionStore = new mySQLStore(session_store_options, pool);
        ...
        async function getUserById(id){
            return (await myQuery("SELECT * FROM users WHERE user_id = ?", [id]))[0];
        }
        ...
        app.get('/', isAuth, async (req, res) => {
            ...
            res.render('index', {username: user.username, xp: user.xp});
        });
        ...
        app.get('/daily_quests', isAuth, async(req, res) => {
            const user = await getUserById(req.session.user_id);
            res.render('daily_quests', {quests: await getAllQuests(user.user_id), 
        ...
        app.get('/tasks', isAuth, async (req, res) => {
            const user = await getUserById(req.session.user_id);
            res.render('tasks', {username: user.username, xp: user.xp});
        });
...
async function createTableDailyQuests(){
...
async function createTableDailyQuestsComp(){
